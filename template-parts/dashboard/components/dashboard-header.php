...
<?php if (!$w_bare):
    ...
    <header class="w-top">
        ...
        <div class="w-top__actions">
            <?php
            // WhatsApp connection at a glance; hidden until a number is added on the WhatsApp page.
            $w_wa = function_exists('sc_wabot_overall_state') ? sc_wabot_overall_state() : array('state' => 'off');
            if ($w_wa['state'] !== 'off'):
                $w_wa_text = array(
                    'ok'      => sc_t('whatsapp.chip_ok', 'WhatsApp connected'),
                    'partial' => sprintf(sc_t('whatsapp.chip_partial', 'WhatsApp: %1$d of %2$d numbers connected'), $w_wa['connected'], count($w_wa['numbers'])),
                    'down'    => sc_t('whatsapp.chip_down', 'WhatsApp disconnected'),
                    'unknown' => sc_t('whatsapp.chip_unknown', 'WhatsApp status not checked recently'),
                )[$w_wa['state']];
                ?>
            <a class="w-top__btn w-wa-chip w-wa-chip--<?php echo esc_attr($w_wa['state']); ?>" href="<?php echo esc_url($dashboard_url . 'whatsapp'); ?>" title="<?php echo esc_attr($w_wa_text); ?>" aria-label="<?php echo esc_attr($w_wa_text); ?>">
                ...
                <span class="w-wa-chip__dot" aria-hidden="true"></span>
            </a>
            <?php endif; ?>
            <?php if ($w_event): ?>
