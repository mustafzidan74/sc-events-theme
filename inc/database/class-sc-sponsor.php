<?php
/**
 * SC Sponsor Model Class
 *
 * Data Access Layer for Sponsors table
 *
 * @package sc_events
 * @version 1.0.0
 */
...
class SC_Sponsor {
    ...
    public static function get_table() {
        global $wpdb;
        if (!self::$table) {
            self::$table = $wpdb->prefix . 'sc_sponsors';
        }
        return self::$table;
    }
    ...
    public static function get_pivot_table() {
        global $wpdb;
        return $wpdb->prefix . 'sc_event_sponsors';
    }
    ...
    public static function get_by_event($event_id) {
        ...
        $sponsors = $wpdb->get_results($wpdb->prepare(
            ...
            FROM $table s
            INNER JOIN $pivot es ON s.id = es.sponsor_id
            WHERE es.event_id = %d
            ORDER BY
    ...
    public static function get_all($args = array()) {
        ...
        if ($args['event_id']) {
            $pivot = self::get_pivot_table();
            $join = "INNER JOIN $pivot es ON s.id = es.sponsor_id AND es.event_id = " . intval($args['event_id']);
        }
        ...
        $sponsors = $wpdb->get_results($wpdb->prepare($sql, $values));
    ...
    public static function create($data) {
        ...
        if ($result) {
            ...
            do_action('sc_sponsor_created', $sponsor_id, $data);
            return $sponsor_id;
        }
    ...
    public static function update($id, $data) {
        ...
        if ($result !== false) {
            do_action('sc_sponsor_updated', $id, $data);
            return true;
        }
    ...
    public static function delete($id) {
        ...
        $wpdb->delete($pivot, array('sponsor_id' => $id), array('%d'));

        $result = $wpdb->delete($table, array('id' => $id), array('%d'));

        if ($result) {
            do_action('sc_sponsor_deleted', $id, $sponsor);
            return true;
        }
     ...
     */
    public static function attach_to_event($sponsor_id, $event_id, $tier_override = null, $sort_order = 0) {
        global $wpdb;
        $pivot = self::get_pivot_table();

        $exists = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $pivot WHERE sponsor_id = %d AND event_id = %d",
            $sponsor_id,
            $event_id
        ));
        ...
        if ($exists) {
            return $wpdb->update(
                ...
                array('sponsor_id' => $sponsor_id, 'event_id' => $event_id)
            ) !== false;
        }
     ...
     */
    public static function detach_from_event($sponsor_id, $event_id) {
        global $wpdb;
        $pivot = self::get_pivot_table();
     ...
     */
    public static function sync_event_sponsors($event_id, $sponsors) {
        global $wpdb;
        $pivot = self::get_pivot_table();
        ...
        foreach ($sponsors as $key => $value) {
            if (is_array($value)) {
                ...
                $sponsor_id = intval($value);
                $tier_override = null;
                $order = $sort_order;
            }

            if ($sponsor_id > 0) {
                self::attach_to_event($sponsor_id, $event_id, $tier_override, $order);
            }
            $sort_order++;
        }
    ...
    /**
     * Get event count for a sponsor
     */
    public static function get_events_count($sponsor_id) {
        global $wpdb;
        $pivot = self::get_pivot_table();

        return (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $pivot WHERE sponsor_id = %d",
            $sponsor_id
        ));
    }
    ...
    private static function prepare_data($data) {
        ...
        // String fields
        $string_fields = array('name', 'title', 'slug', 'phone', 'tier');
    ...
    /**
     * Hydrate sponsor object
     */
    private static function hydrate($sponsor) {
        // Logo URL
        if (!empty($sponsor->logo)) {
            $sponsor->logo_url = wp_get_attachment_image_url($sponsor->logo, 'medium');
            $sponsor->logo_url_large = wp_get_attachment_image_url($sponsor->logo, 'large');
        } else {
            $sponsor->logo_url = '';
            $sponsor->logo_url_large = '';
        }

        // Tier label
        $sponsor->tier_label = self::get_tier_label($sponsor->tier);
