 ...
 */
add_action('wp_ajax_sc_get_certificates_paginated', 'sc_get_certificates_paginated');
function sc_get_certificates_paginated() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $certificates = SC_Certificate::get_all($args);
    $total = SC_Certificate::count($args);
    $total_pages = ceil($total / $per_page);

    // Add event_deleted flag for orphaned certificates (soft-deleted or missing events)
    $visible_statuses = array('publish', 'draft', 'completed');
    foreach ($certificates as &$cert) {
        $eid = is_array($cert) ? ($cert['event_id'] ?? 0) : ($cert->event_id ?? 0);
        $ev = !empty($eid) ? SC_Event::get($eid) : null;
        $is_deleted = !$ev || !in_array($ev->status ?? '', $visible_statuses);
        if (is_array($cert)) {
            $cert['event_deleted'] = $is_deleted;
        } else {
            $cert->event_deleted = $is_deleted;
        }
    }
 ...
 */
add_action('wp_ajax_sc_get_certificate', 'sc_get_certificate');
function sc_get_certificate() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    if (!$certificate_id) {
        wp_send_json_error(array('message' => __('Certificate ID is required.', 'sc_events')));
    }

    $certificate = SC_Certificate::get($certificate_id);

    if (!$certificate) {
        wp_send_json_error(array('message' => __('Certificate not found.', 'sc_events')));
    }

    wp_send_json_success(array('certificate' => $certificate));
}

/**
 * Issue certificate to single attendee
 */
add_action('wp_ajax_sc_issue_certificate', 'sc_issue_certificate');
function sc_issue_certificate() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $existing = SC_Certificate::get_by_attendee_event($attendee_id, $event_id);
    if ($existing) {
        wp_send_json_error(array(
            'message' => __('Certificate already issued to this attendee for this event.', 'sc_events'),
    ...
    // Get template
    if (!$template_id) {
        // Try to get event's template or default
...
/**
 * Bulk issue certificates for event attendees
 */
add_action('wp_ajax_sc_bulk_issue_certificates', 'sc_bulk_issue_certificates');
function sc_bulk_issue_certificates() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $event_id = isset($_POST['event_id']) ? intval($_POST['event_id']) : 0;
    $template_id = isset($_POST['template_id']) ? intval($_POST['template_id']) : 0;
    $attendee_ids = isset($_POST['attendee_ids']) ? array_map('intval', (array) $_POST['attendee_ids']) : array();
    $checked_in_only = isset($_POST['checked_in_only']) ? (bool) $_POST['checked_in_only'] : false;
    ...
    if (empty($attendee_ids)) {
        ...
        // Get specific attendees
        $attendees = array();
        foreach ($attendee_ids as $id) {
            $attendee = SC_Attendee::get($id);
            if ($attendee) {
                $attendees[] = $attendee;
            }
        }
    ...
    if ($checked_in_only) {
        $attendees = array_filter($attendees, function($attendee) {
            ...
            $checked_in = is_array($attendee) ? ($attendee['checked_in'] ?? false) : ($attendee->checked_in ?? false);
            return !empty($checked_in);
        });
    }

    if (empty($attendees)) {
        wp_send_json_error(array('message' => __('No eligible attendees found.', 'sc_events')));
    }

    // Extract IDs from filtered attendees (bulk_issue expects array of IDs, not objects)
    $filtered_ids = array_map(function($attendee) {
        // Handle both array and object access
        $id = is_array($attendee) ? ($attendee['id'] ?? 0) : ($attendee->id ?? 0);
        return intval($id);
    }, $attendees);

    $result = SC_Certificate::bulk_issue($event_id, $template_id, $filtered_ids);

    wp_send_json_success(array(
        'message' => sprintf(
            __('Certificates issued: %d. Skipped (already issued): %d.', 'sc_events'),
            $result['issued'],
            $result['skipped']
        ),
 ...
 * Revoke certificate
 */
add_action('wp_ajax_sc_revoke_certificate', 'sc_revoke_certificate');
function sc_revoke_certificate() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $result = SC_Certificate::revoke($certificate_id, $reason);

    if ($result) {
        wp_send_json_success(array('message' => __('Certificate revoked successfully.', 'sc_events')));
    } else {
        wp_send_json_error(array('message' => __('Failed to revoke certificate.', 'sc_events')));
    }
}
 ...
 */
add_action('wp_ajax_sc_reinstate_certificate', 'sc_reinstate_certificate');
function sc_reinstate_certificate() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    if ($result !== false) {
        wp_send_json_success(array('message' => __('Certificate reinstated successfully.', 'sc_events')));
    } else {
        wp_send_json_error(array('message' => __('Failed to reinstate certificate.', 'sc_events')));
    }
}

/**
 * Get event certificate statistics
 */
add_action('wp_ajax_sc_get_event_certificate_stats', 'sc_get_event_certificate_stats');
function sc_get_event_certificate_stats() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $stats = SC_Certificate::get_event_stats($event_id);

    // Get total attendees for the event
    $total_attendees = SC_Attendee::count(array('event_id' => $event_id));
    $stats['total_attendees'] = $total_attendees;
    $stats['pending'] = $total_attendees - $stats['total'];
 ...
 */
add_action('wp_ajax_sc_send_certificate_email', 'sc_send_certificate_email');
function sc_send_certificate_email() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    if (!$attendee || empty($attendee['email'])) {
        wp_send_json_error(array('message' => __('Attendee email not found.', 'sc_events')));
    }

    // Prepare email
    $subject = sprintf(
        __('Your Certificate for %s', 'sc_events'),
        $certificate['event_title']
    );

    $download_url = SC_Certificate::get_download_url($certificate_id);
    $verify_url = SC_Certificate::get_verification_url($certificate['verification_code']);

    $message = sprintf(
        __("Dear %s,\n\nCongratulations! Your certificate for \"%s\" is ready.\n\nCertificate Number: %s\n\nYou can download your certificate here:\n%s\n\nVerify certificate authenticity:\n%s\n\nBest regards,\nThe Event Team", 'sc_events'),
    ...
    if ($sent) {
        SC_Certificate::mark_email_sent($certificate_id);
 ...
 */
add_action('wp_ajax_sc_bulk_send_certificate_emails', 'sc_bulk_send_certificate_emails');
function sc_bulk_send_certificate_emails() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'sc_dashboard_nonce')) {
        wp_send_json_error(array('message' => __('Security check failed.', 'sc_events')));
    }
    ...
    $certificate_ids = isset($_POST['certificate_ids']) ? array_map('intval', (array) $_POST['certificate_ids']) : array();
    $event_id = isset($_POST['event_id']) ? intval($_POST['event_id']) : 0;
    $unsent_only = isset($_POST['unsent_only']) ? (bool) $_POST['unsent_only'] : true;

    if (empty($certificate_ids) && !$event_id) {
        wp_send_json_error(array('message' => __('Certificate IDs or Event ID required.', 'sc_events')));
    }

    // If event_id provided, get all certificates for event
    if ($event_id && empty($certificate_ids)) {
        $args = array(
            'event_id' => $event_id,
            'status' => 'issued',
            'per_page' => -1
        ...
        $certificates = SC_Certificate::get_all($args);
        $certificate_ids = array_column($certificates, 'id');
    }

    if (empty($certificate_ids)) {
        wp_send_json_error(array('message' => __('No certificates to send.', 'sc_events')));
    }
    ...
    foreach ($certificate_ids as $cert_id) {
        ...
        $attendee = SC_Attendee::get($certificate['attendee_id']);
        // Handle both array and object access for attendee
        $attendee_email = is_array($attendee) ? ($attendee['email'] ?? '') : ($attendee->email ?? '');
        if (!$attendee || empty($attendee_email)) {
            $failed++;
            continue;
        }
        ...
        $download_url = SC_Certificate::get_download_url($cert_id);
        $verify_url = SC_Certificate::get_verification_url($certificate['verification_code']);
        ...
        if (wp_mail($attendee_email, $subject, $message, $headers)) {
            SC_Certificate::mark_email_sent($cert_id);
            $sent++;
        } else {
            $failed++;
        }
    ...
    wp_send_json_success(array(
        'message' => sprintf(
            __('Emails sent: %d. Failed: %d.', 'sc_events'),
            $sent,
            $failed
        ),
        'sent' => $sent,
...
function sc_download_certificate_handler() {
    ...
    if (!$has_access && is_user_logged_in()) {
        $attendee = SC_Attendee::get($certificate['attendee_id']);
        if ($attendee && $attendee['user_id'] == get_current_user_id()) {
            $has_access = true;
        }
    }
    ...
    if ($event) {
        ...
        // Get organizer
        if (!empty($event['organizer_ids'])) {
            $organizer_id = is_array($event['organizer_ids']) ? $event['organizer_ids'][0] : $event['organizer_ids'];
            $organizer = get_userdata($organizer_id);
            if ($organizer) {
                $data['organizer_name'] = $organizer->display_name;
            }
        }
    }
